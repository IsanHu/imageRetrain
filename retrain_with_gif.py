  ...
  for label_name, label_lists in image_lists.items():
    ...
    for category in ['training', 'testing', 'validation']:
      ...
      for index, unused_base_name in enumerate(category_list):
        ...
        how_many_label_bottlenecks += 1
        if how_many_bottlenecks % 1000 == 0:
          print(str(how_many_bottlenecks) + ' bottleneck files created.')
    print(label_name + "的: " + str(how_many_label_bottlenecks) + ' bottleneck files created.')
  print("总共" + str(how_many_bottlenecks) + ' bottleneck files created.')
...
def main(_):
  # TensorBoard 用于可视化机器学习的过程
  if tf.gfile.Exists(FLAGS.summaries_dir):
    tf.gfile.DeleteRecursively(FLAGS.summaries_dir)
  tf.gfile.MakeDirs(FLAGS.summaries_dir)

  # 加载 Inception 模型
  # bottleneck 是在最后输出层之前的一层的非正式称呼
  ...
  for i in range(FLAGS.how_many_training_steps):
    ...
    # 每循环训练{FLAGS.eval_step_interval}次，评估一次训练效果
